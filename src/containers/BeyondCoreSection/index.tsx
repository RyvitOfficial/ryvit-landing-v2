...
import { moreTools } from '../../constants';

const BeyondCoreSection = () => {
  return (
    <CWrapper id="beyond" className="flex flex-col items-center justify-center py-12 relative">
      ...
      <CContainer>
        <div className="max-w-6xl mx-auto text-center mb-16">
          <CSectionBadge title="Beyond the Core" />
      ...
      <CContainer>
        <div className="flex flex-col items-center justify-center py-10 md:py-20 w-full">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 w-full divide-x-0 md:divide-x divide-y md:divide-y-0 divide-gray-900 first:border-l-2 lg:first:border-none first:border-gray-900">
            {moreTools.map((moreTools) => (
              <div
                key={moreTools.title}
                className="flex flex-col items-start px-4 md:px-6 lg:px-8 lg:py-6 py-4"
              >
                <div className="flex items-center justify-center">
                  <moreTools.icon className="w-8 h-8" />
                </div>
                <h3 className="text-lg font-medium mt-4">{moreTools.title}</h3>
